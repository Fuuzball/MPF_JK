...
class MPF_GlassThird(MPF_Glass):
    ...
    def K_dK(self, JbK):
        x = self.X
        J, b, g = self.reshape_JbK(JbK)

        dE = 2 * x * (x @ J) - 2 * x * b[None, :] # From J, b

        dE = 2 * 3 * np.einsum('ijd,ni,nj,nd -> nd', g, x, x, x)
        dE += -4 * 3 * np.einsum('idd,ni -> nd', g, x)
        dE += 8 * np.einsum('ddd,nd -> nd', g, x)
